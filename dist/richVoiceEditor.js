...
var RichVoiceEditor = function (_Module) {
    ...
    function RichVoiceEditor(quill, options) {
        ...
        // template for additional closed SSML tags (position)
        /*
            // SSML template tag
            // --------------
            // style toolbar button with icon
            document.querySelector('.ql-ssml_template').innerHTML = '<i class="template icon" title="Template"></i>';
            function ssml_template_handler(value) {
                // only if a position is currently selected
                if (value) {
                    // get the current position idndex
                    const cursorPosition = this.quill.getSelection().index;
                    // insert the single tag
                    this.quill.insertText(cursorPosition, '<template/>');
                    // set cursor position to the end of new tag
                    this.quill.setSelection(cursorPosition + value.length + 20);
                }
            }
            // add tag handler to quill toolbar
            toolbar.addHandler('ssml_template', ssml_template_handler.bind(this));
        */

        // template for additional SSML tags (range)
        /*
            // SSML template tag
            // --------------
            // style toolbar button with icon
            document.querySelector('.ql-ssml_template').innerHTML = '<i class="template icon" title="Template"></i>';
            function ssml_template_handler() {
                // get current selected text as range
                var range = quill.getSelection();
                // only if range is currently selected
                if (range) {
                    ...
                    if (range.length > 0) {
                        // add tag at the end of the selected range
                        this.quill.insertText(range.index + range.length, '</template>')
                        // add tag at the beginning of the selected range
        ...
        */
        ...
        // template for additional dropdown SSML tags
        /*
            // SSML template tag
            // ---------------
            // load texts for dropdown items
            const templatePickerItems = Array.prototype.slice.call(document.querySelectorAll('.ql-ssml_template .ql-picker-item'));
            templatePickerItems.forEach(item => item.textContent = item.dataset.value);
            // style toolbar button with icon and keep dropdown values
            document.querySelector('.ql-ssml_template .ql-picker-label').innerHTML = '<i class="template icon" title="Template"></i>' + document.querySelector('.ql-ssml_template .ql-picker-label').innerHTML;
            // fix withd and padding problem in quill toolbar for dropdowns
            document.querySelector('.ql-ssml_template').style.width = '45px';
            document.querySelector('.ql-ssml_template').style.padding = '4px 0 0 0';
            function ssml_sayas_handler(value) {
                // get current selected text as range
                var range = quill.getSelection();
                // only if range is currently selected
                if (range) {
                    ...
                    if (range.length > 0) {
                        ...
                        this.quill.insertText(range.index + range.length, '</template>')
                        // add tag at the beginning of the selected range
                        this.quill.insertText(range.index, '<template template-attribute="' + value + '">')
                        // set cursor position to the end of new tag
                        this.quill.setSelection(range.index + range.length + value.length + 33);
                    }
                }
